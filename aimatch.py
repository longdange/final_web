...
def predict_with_voting(container, repeat=5):
    ...
    for i in range(0, len(axis_prompts), 2):
        ...
        for _ in range(repeat):
            indices = sample_frame_indices(clip_len, 1, total_frames)
            ...
            with torch.no_grad():
                logits = model(**inputs).logits_per_video[0].cpu().numpy()
                left_scores.append(logits[0])
                right_scores.append(logits[1])

        final_code += left_code if np.mean(left_scores) > np.mean(right_scores) else right_code
...
if not uploaded_video:
    st.info("⬆️ 위에 mp4 비디오 파일을 업로드해주세요.")
...
with st.form("dog_form", clear_on_submit=False):
    ...
    admission_date = st.date_input("입소일", value=date.today(), min_value=date(2000, 1, 1))
    breed = st.text_input("품종")
    age = st.number_input("나이(개월)", min_value=0, step=1)
    birth_date = st.date_input("생년월일", value=date.today(), min_value=date(2000, 1, 1))
    ...
    if st.form_submit_button("➕ 추가하기"):
        new_row = pd.DataFrame([[
            shelter_name, admission_date.strftime("%Y-%m-%d"), breed, age,
            birth_date.strftime("%Y-%m-%d"), gender, weight, health_status,
            vaccination, color_pattern, neutered, dbti_code, dbti_desc
